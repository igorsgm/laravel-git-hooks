...
test(
    'Gets list of changed files', function () {
        chdir(__DIR__);
...
test(
    'Gets last commit text from git log', function () {
        chdir(__DIR__);
...
test(
    'Throws ProcessFailedException on errored git command', function () {
...
test(
    'Gets Commit message content from .git file', function () {
...
test(
    'Updates Commit message content in .git file', function () {
        $gitFileName = 'COMMIT_MESSAGE';
        $messageText = 'test message';
...
test(
    'isMergeInProgress returns true when a merge is in progress', function () {
...
test(
    'isMergeInProgress returns false when a merge is not in progress', function () {
        chdir(base_path());
