...
test(
    'Skips Enlightn check if there are no files added to commit', function () {
...
test(
    'Fails commit when Enlightn is not passing', function ($listOfChangedFiles) {
        $this->config->set(
...
test(
    'Commit passes when Enlightn check is OK', function ($listOfChangedFiles) {
        $this->config->set(
            'git-hooks.pre-commit', [
                EnlightnPreCommitHook::class,
            ]
